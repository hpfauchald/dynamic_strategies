...
def stats(R_ts, Rf, weights, Rebalancing):
    ...
    geom_avg_xs_return = (
        geom_avg_total_return - geom_avg_rf
    )  # final_pt_val_xs**(Rebalancing / n_periods) - 1
...
def compute_turnover(previous_weights, new_weights, asset_returns, rf_return):
    ...
    Rp = (
        np.dot(previous_weights, asset_returns)
        + (1 - np.sum(previous_weights)) * rf_return
    )
...
def kelly(
    ...
    if expected_return == "known":
        ...
        pred_E_R = (
            pd.Series(r)
            .shift(1)
            .rolling(window=estimation_period * periods)
            .mean()
            .to_numpy()
        )
    ...
    return (
        {
            ...
            "Volatility Targeting Final Log Wealth": np.log(
                res["final_wealth_volTarget"]
            ),
        ...
        np.log(res["final_wealth_kelly"]),
        np.log(res["final_wealth_long"]),
        np.log(res["final_wealth_volTarget"]),
    )
